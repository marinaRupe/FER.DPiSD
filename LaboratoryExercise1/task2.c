...
typedef double (*PtrFunDUfD)(Unary_Function*, double);

double Unary_Function_negative_value_at(Unary_Function* obj, double x) {
    PtrFunDUfD value_at = (PtrFunDUfD) obj->vtable[0];
    return -value_at(obj, x);
}

PTRFUN Unary_Function_Vtable[2] = {
        (PTRFUN) NULL,
        (PTRFUN) Unary_Function_negative_value_at,
};

void Unary_Function_Init(Unary_Function* obj, int lb, int ub) {
    obj->lower_bound_ = lb;
    obj->upper_bound_ = ub;
    obj->vtable = Unary_Function_Vtable;
}
